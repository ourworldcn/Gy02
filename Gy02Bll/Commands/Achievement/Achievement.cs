...
namespace GY02.Commands
{
    ...
    public class AchievementFuhuaedHandler : ISyncCommandHandled<FuhuaCommand>
    {
        ...
        public void Handled(FuhuaCommand command, Exception exception = null)
        {
            ...
            _AchievementManager.RaiseEventIfChanged(Guid.Parse("401cbe25-ed70-4a43-a5a8-524cf57f56c2"), 1, command.GameChar, now);
        }
    }
    ...
    public class LvUped : ISyncCommandHandled<LvUpCommand>
    {
        ...
        public void Handled(LvUpCommand command, Exception exception = null)
        {
            ...
            if (command.Ids.Count > 0)
                _AchievementManager.RaiseEventIfChanged(Guid.Parse("f7647e8e-cdf1-4a30-982e-fb676cfc6c34"), command.Ids.Count, command.GameChar, now);
    ...
    public class ChoujiangBaoxiangClass : ISyncCommandHandled<ShoppingBuyCommand>
    {
        ...
        public void Handled(ShoppingBuyCommand command, Exception exception = null)
        {
            ...
            if (_ShoppingManager.GetShoppingTemplateByTId(command.ShoppingItemTId) is TemplateStringFullView tt)
            {
                ...
                {
                    _AchievementManager.RaiseEventIfChanged(Guid.Parse("4963c720-2b8f-4def-aed2-7bc8925f6a91"), command.Count, command.GameChar, OwHelper.WorldNow);
                    _AchievementManager.RaiseEventIfChanged(Guid.Parse("5173f53e-6534-4594-82cc-d989f52f03c7"), command.Count, command.GameChar, OwHelper.WorldNow);
                    //f9ec4736-d926-43a6-98fb-44d1adb11faf	每日任务-子任务1（开宝箱次数）
                    _AchievementManager.RaiseEventIfChanged(Guid.Parse("f9ec4736-d926-43a6-98fb-44d1adb11faf"), command.Count, command.GameChar, OwHelper.WorldNow);
                }
                ...
                {
                    _AchievementManager.RaiseEventIfChanged(Guid.Parse("6020445b-ff60-4a8d-96b3-78e79f56fcf6"), command.Count, command.GameChar, OwHelper.WorldNow);
                }
            }
        }
    }
